...
class PointNet(torch.nn.Module):
    def __init__(self, train_with_norm=False):
        ...
        mlp1 = Sequential(Linear(in_channels + 4, out_channels),
                          ReLU(),
                          Linear(out_channels, out_channels))
        self.conv1 = PPFConv(local_nn=mlp1)

        mlp2 = Sequential(Linear(out_channels + 4, out_channels),
                          ReLU(),
                          Linear(out_channels, out_channels))
        self.conv2 = PPFConv(local_nn=mlp2)

        mlp3 = Sequential(Linear(out_channels + 4, out_channels),
                          ReLU(),
                          Linear(out_channels, out_channels))
        self.conv3 = PPFConv(local_nn=mlp3)

        self.classifier = Classifier(in_channels=out_channels,hidden_channels=128)

    def forward(self, pos, batch, normal):
