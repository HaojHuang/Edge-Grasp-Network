...
from data_robot import edge_grasper4
...
def main(args):
    ...
    grasper = edge_grasper4.EdgeGrasper(device=args.device,root_dir='./data_robot/store15',lr=0.5*1e-4,load=args.load)
            ...
            for partial_view in range(NUMBER_VIEWS):
                ...
                for _ in range(GRASPS_TRIAL_SCENE):
                    ...
                    if args.baseline:
                        ...
                        data = data.to(device)
                        scores, scores_colli, scores_drop,depth_projection, sample_normal, des_normals, sample_pos, \
                        radius_p_index = grasper.model.act(data)

                        scores = scores.squeeze(dim=-1)
                        scores_colli = scores_colli.squeeze(dim=-1)
                        scores_drop = scores_drop
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load", type=int, default=70)
