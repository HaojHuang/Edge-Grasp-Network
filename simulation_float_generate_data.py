...
class ClutterRemovalSim(object):
    ...
    def reset(self, object_count):
        self.world.reset()
        self.world.set_gravity([0.0, 0.0, -9.81+9.81])
        #self.draw_workspace()
        ...
        if self.scene == "pile":
            urdf,pose = self.generate_pile_scene(object_count, table_height,True)
            return urdf,pose
    ...
    def generate_pile_scene(self, object_count, table_height,return_urdf=False):
        # place box
        ...
        #print(self.object_urdfs)
        # urdfs = self.object_urdfs[:1]
        # print(urdfs, '==========================')
        #urdfs = [ Path('data_robot/urdfs/pile/train/Tortoise_800_tex.urdf')]
        #Path('data_robot/urdfs/pile/train/v8_fusion_peach_mango.urdf')
        #urdfs = [Path('data_robot/urdfs/pile/train/starfruit_poisson_000.urdf')]
        #Path('data_robot/urdfs/pile/train/cellphone_poisson_032.urdf')
        for urdf in urdfs:
    ...
    def execute_grasp(self, grasp, remove=True, allow_contact=False):
        ...
        if self.gripper.detect_contact():
            ...
            if self.gripper.detect_contact() and not allow_contact:
                ...
                # pybullet.changeDynamics(self.gripper.body.uid, 0, lateralFriction=0.75, spinningFriction=0.05)
                # pybullet.changeDynamics(self.gripper.body.uid, 1, lateralFriction=0.75, spinningFriction=0.05)
                ...
                #print('calculating grasping pose', T_world_grasp.translation)
                #print('world gripper pose', self.gripper.body.get_pose().translation)
                #print('========================================================')
                pybullet.changeDynamics(self.gripper.body.uid, 0, lateralFriction=0.75, spinningFriction=0.05)
                pybullet.changeDynamics(self.gripper.body.uid, 1, lateralFriction=0.75, spinningFriction=0.05)
                self.gripper.move(0.0)
                # add gravity and shake the hand
                #print(self.gripper.body.links)
                #print(self.gripper.body.links['panda_leftfinger'].link_index,self.gripper.body.links['panda_rightfinger'].link_index)
                #dynamics_info = pybullet.getDynamicsInfo(self.gripper.body.uid, 0,)
                #dynamics_info2 = pybullet.getDynamicsInfo(self.gripper.body.uid, 1)
                #print(dynamics_info)
                #print(dynamics_info2)
                #pybullet.changeDynamics(self.gripper.body.uid, 0, lateralFriction=10, spinningFriction=10)
                #pybullet.changeDynamics(self.gripper.body.uid, 1, lateralFriction=10, spinningFriction=10)
                self.advance_sim(10)
                self.world.set_gravity([0.0, 0.0, -9.81])
                #self.gripper_dance()
                self.advance_sim(20)
                ...
                if self.check_success(self.gripper):
                    #print('success')
                    #object_id = self.gripper.grasp_object_id()
                    #print(object_id)
                    dis_from_hand = self.gripper.get_distance_from_hand()# the distance between the object and the gripper
                    #print(dis_from_hand)
                    #print(self.gripper.is_dropped(object_id,dis_from_hand))
                    self.gripper.move_tcp_xyz(T_world_retreat, abort_on_contact=False)
                    #print('finish extracting the object')
                    self.gripper.move_gripper_top_down()
                    #self.gripper.shake_hand(dis_from_hand)
                    #print('finish topdown moving')
                    shake_label = False
                    if self.check_success(self.gripper):
                        shake_label = self.gripper.shake_hand(dis_from_hand)
                        #print('finish shaking')
...
class Gripper(object):
    ...
    def __init__(self, world):
        ...
        self.T_body_tcp = Transform(Rotation.identity(), [0.0, 0.0, 0.0]) # the gripper center frame will be
    ...
    def move_gripper_top_down(self):
        ...
        pos = current_pose.translation + 0.05
