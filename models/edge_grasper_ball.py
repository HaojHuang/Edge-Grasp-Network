import os.path
from dataset_implicit_approach_efficiency import Grasp_Dataset, GraspNormalization, GraspAugmentation, PreTransformBallBox
from edge_grasp_ball_rss2 import EdgeGrasp
import torch
...
class EdgeGrasper:
    ...
    def train_test_save(self, train_dataset, test_dataset, tr_epoch=200, verbose=False, test_interval=1,save_interval=100,log=True):
        #time0 = time.time()
...
parser.add_argument('--dataset_dir', type=str, default='./clutter_max')
...
def main(args):
    ...
    # set up the dataset
    tr_dataset = Grasp_Dataset(root=args.dataset_dir, pre_transform=PreTransformBallBox(max_width=True),transform=Compose([GraspNormalization()]), train=True)
    ...
    # set up the model
    edge_grasper = EdgeGrasper(device=1,root_dir='./egde_clutter_max2_noaug', sample_num=args.sample_num, lr=1e-7, load=args.load)
