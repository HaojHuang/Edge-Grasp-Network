from vn_pointnet_graph_improved import PointNetSimpleVn,GlobalEmdModelVn,Classifier,VNStdFeature
import torch
...
class EdgeGrasp:
    def __init__(self, device, sample_num=32, lr= 0.5* 1e-3, ubn=False, normal=True, aggr='max'):
        ...
        self.global_emd_model = GlobalEmdModelVn(input_c=32+64+128, output_c=256, ubn=ubn, aggr=aggr).to(device)
        self.std = VNStdFeature(in_channels=387, normalize_frame=False, negative_slope=0.0, ubn=ubn).to(device)
        self.classifier_fail = Classifier(in_channels=1161, hidden_channels=(512, 256, 128), ubn=False).to(
            device)
